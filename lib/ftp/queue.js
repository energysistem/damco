...
exports.add = function (filename) {
    getId(function (err, id) {
        if (!err) {
            var request = new sql.Request(db),
                query = "INSERT INTO TRAMAS (ID, TIPO, F_CREA, F_PROGRAMADO, TRAMA) VALUES ('" + id + "', 'DAMCO', GETDATE(), GETDATE(), '" + filename + "');";
...
exports.get = function (req, res, next) {
    var request = new sql.Request(db),
        ...
        query = "SELECT TOP 30 ID as id, TRAMA as name FROM TRAMAS WHERE TIPO='DAMCO' AND F_EJECUTADO IS NULL AND F_PROGRAMADO < GETDATE() AND TRAMA NOT LIKE 'artconf_%' ";
